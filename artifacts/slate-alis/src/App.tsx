...
// Grade R is stored as 0 (the reception year below Grade 1).
const gradeLabel = (grade: number) => (grade === 0 ? 'Grade R' : `Grade ${grade}`);
...
function Assignments() {
  ...
  return <><PageIntro eyebrow="Your work" title="Assignments" detail="Open when you are ready. Locked when the timing matters. Every status tells you what to do next." /><JoinClassCard /><div className="mb-6 flex flex-wrap gap-2">{['ALL', 'OPEN', 'LOCKED', 'SUBMITTED', 'CLOSED', 'MISSED'].map((item) => <button key={item} onClick={() => setFilter(item)} data-testid={`button-filter-${item.toLowerCase()}`} className={cn('rounded-full border px-3 py-2 text-xs font-bold transition-colors', filter === item ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--accent))]')}>{item === 'ALL' ? 'All work' : item[0] + item.slice(1).toLowerCase()}</button>)}</div>{!assignments.length ? <EmptyState icon={<BookOpen size={21} />} title="Nothing assigned yet" detail="When your teacher shares work, it will appear here." /> : !filtered.length ? <EmptyState icon={<FileQuestion size={21} />} title="No assignments in this view" detail="Try another status to find what you are looking for." /> : <div className="grid gap-4 md:grid-cols-2">{filtered.map((assignment, index) => <AssignmentCard key={assignment.id} assignment={assignment} index={index} />)}</div>}</>;
}
